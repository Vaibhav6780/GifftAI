...
import type { TelegramInboxConversation } from "@gifftai/shared";
import { useIntegrationsList, useSendTelegramReply, useTelegramInbox } from "../../features/integrations/api";
...
export function TelegramInboxPage() {
  ...
  const { data, isLoading, isError, refetch } = useTelegramInbox({
    page: 1,
    pageSize: 50,
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] flex-col gap-4">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Telegram Inbox</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">Every Telegram conversation, in one place.</p>
      </div>
      ...
      <div className="flex min-h-0 flex-1 gap-4">
        <Card className="flex w-80 shrink-0 flex-col overflow-hidden p-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              <EmptyState icon={Send} title="No Telegram conversations yet" />
            ) : (
              <ul>
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="truncate text-sm font-medium text-slate-900 dark:text-slate-100">
                          {conversation.leadName || (conversation.username ? `@${conversation.username}` : "Unknown")}
                        </span>
        ...
        <Card className="flex min-h-0 flex-1 flex-col p-0">
          {selectedConversation ? (
            <TelegramThreadPanel conversation={selectedConversation} />
          ) : (
            <div className="flex flex-1 items-center justify-center">
              <EmptyState
                icon={Send}
                title="Select a conversation"
                description="Pick a lead on the left to view its Telegram history."
              />
            </div>
          )}
        </Card>
...
function TelegramThreadPanel({ conversation }: { conversation: TelegramInboxConversation }) {
  const { data: entries, isLoading } = useLeadTimeline(conversation.leadId);
  const { data: integrations } = useIntegrationsList();
  const sendReply = useSendTelegramReply(conversation.leadId);
  const canUpdate = useHasPermission("leads:update");
  const [replyBody, setReplyBody] = useState("");

  const telegramConnected = integrations?.some((i) => i.channelType === "TELEGRAM" && i.status === "CONNECTED");
  const canReply = canUpdate && telegramConnected;
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-slate-200 p-4 dark:border-slate-800">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          {conversation.username ? `@${conversation.username}` : `Chat ${conversation.chatId}`}
        </p>
      </div>
      ...
      {canReply && (
        <div className="flex flex-col gap-2 border-t border-slate-200 p-3 dark:border-slate-800">
          <Textarea
            placeholder="Message this lead on Telegram…"
            rows={2}
            value={replyBody}
